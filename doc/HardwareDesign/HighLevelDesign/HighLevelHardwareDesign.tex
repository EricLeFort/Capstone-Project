...
\textbf{Description}\\
The Rotational Motor is responsible for properly orienting the EE in the proper direction. It will spin about the axis of the EEA which is attached to the EE at its bottom end. The Rotational Motor will be housed in the EEB since it provides the most support for a high torque motor. Furthermore, the shaft of the motor will be pointing straight down towards the table to allow the attached EE to reached closest to the table without ball interference. Once the proper orientation is acheived it will remain firmly in place to allow the EE to maintain accuracy whilst attempting a shot. There will be no belts required, only the use of steps or another feedback mechanism that will track the angle relative to the x-axis. \\\\
\textbf{Requirements}\\
Since the motor's responsibilities are simple, there are not many requirements applied. First, the motor is required to spin a minimum of 360 degrees in order to cover all the possible orientation angles. The accuracy of this positioning must be sufficient so that an orientation is achieved within the available errors of angular direction, otherwise no final position may be met. As briefly mentioned in the description section above, the motor must resist disturbances from the EE so that its accuracy may be maintained. Although a stepper motor is prefered other motors may be used as long as they have the ability to track rotational displacement. 
